#!/bin/bash
# Revision Date: 12/10/2016
# This script will apply traffic shaping on eth1 interface of LAB LAN, 
...
# Define you link characteristics below
...
deviation1=100ms
packet_loss1=3%
# The IP of the SmartBox WAN1 port
...
deviation2=60ms
packet_loss2=0%
# The IP of the SmartBox WAN2 port
...
if [ "${1}" = "clear" ];then
	...
	for DEV in $SAT1;do
		...
		#	tc qdisc add dev $DEV parent 1:11 handle 101: sfq perturb 10 || echo error at line ${LINENO}
		#	tc qdisc add dev $DEV parent 1:12 handle 102: sfq perturb 10 || echo error at line ${LINENO}
		...
		tc qdisc del dev $DEV ingress > /dev/null 2>&1 || echo error at line $LINENO
		tc qdisc del dev ifb0 root > /dev/null 2>&1 || echo error at line  $LINENO
